# 🚀 PPanel 完整部署指南

前后端同虚拟机部署方案 - GCP 虚拟机 IP: **34.177.90.11**

---

## 📊 架构概览

```
用户浏览器
    ↓
34.177.90.11:3000 (Admin Web)
34.177.90.11:3001 (User Web)
    ↓
34.177.90.11:8080 (API Server)
    ↓
Docker Network (ppanel-network)
    ↓
MySQL (mysql:3306) + Redis (redis:6379)
```

---

## 🎯 部署顺序

### ⚠️ 重要：必须按顺序部署

1. **后端 + 数据库** ← 先部署
2. **前端** ← 后部署（依赖后端 API）

---

## 📦 第一步：部署后端

### 1.1 连接虚拟机

```bash
# SSH 连接到 GCP 虚拟机
gcloud compute ssh your-instance-name
# 或
ssh user@136.110.11.215
```

### 1.2 克隆后端代码

```bash
cd ~
git clone YOUR_BLITZ_ARROW_SERVER_REPO_URL blitz-arrow-server
cd blitz-arrow-server
```

### 1.3 一键部署后端

```bash
chmod +x scripts/deploy-from-source.sh
./scripts/deploy-from-source.sh
```

⏳ **等待 3-5 分钟**，脚本会自动：
1. ✅ 下载 Go 依赖
2. ✅ 编译 Go 应用
3. ✅ 构建 Docker 镜像
4. ✅ 启动 MySQL + Redis + Server 容器

### 1.4 初始化后端

访问：**http://136.110.11.215:8080/init**

填写配置：

| 服务  | 字段     | 值                  |
|-------|----------|---------------------|
| MySQL | 主机地址 | `mysql`             |
|       | 端口     | `3306`              |
|       | 数据库名 | `ppanel`            |
|       | 用户名   | `ppanel`            |
|       | 密码     | `ppanel_password`   |
| Redis | 主机地址 | `redis`             |
|       | 端口     | `6379`              |
|       | 密码     | 留空                |
| 管理员| 邮箱     | 您的邮箱            |
|       | 密码     | 设置安全密码        |

点击提交完成初始化。

---

## 🎨 第二步：部署前端

### 2.1 克隆前端代码

```bash
cd ~
git clone YOUR_BLITZ_ARROW_REPO_URL blitz-arrow
cd blitz-arrow
```

### 2.2 一键部署前端

```bash
chmod +x scripts/deploy-from-source.sh
./scripts/deploy-from-source.sh
```

⏳ **等待 15-20 分钟**，脚本会自动：
1. ✅ 配置环境变量（API 地址等）
2. ✅ 安装 Bun（如果需要）
3. ✅ 安装依赖（bun install）
4. ✅ 构建应用（Turbo + Next.js）
5. ✅ 构建 Docker 镜像
6. ✅ 启动 Admin + User 容器

---

## 🔥 第三步：配置防火墙

### GCP 防火墙规则

```bash
gcloud compute firewall-rules create allow-ppanel-all \
  --allow tcp:3000,tcp:3001,tcp:8080 \
  --description "Allow PPanel Admin, User, and API" \
  --direction INGRESS \
  --target-tags=ppanel
```

或在 [GCP 控制台](https://console.cloud.google.com/networking/firewalls/list) 手动添加：
- **名称**: `allow-ppanel-all`
- **目标**: 网络中的所有实例
- **来源 IPv4 范围**: `0.0.0.0/0`
- **协议和端口**: TCP - `3000,3001,8080`

---

## 🌐 第四步：访问应用

### 应用地址

| 应用 | 地址 | 说明 |
|------|------|------|
| **Admin 管理后台** | http://136.110.11.215:3000 | 管理员使用 |
| **User 用户前端** | http://136.110.11.215:3001 | 普通用户使用 |
| **API 文档** | http://136.110.11.215:8080/swagger | API 接口文档 |

### 登录信息

使用初始化时设置的管理员邮箱和密码登录。

---

## 📊 容器管理

### 查看所有容器

```bash
docker ps
```

应该看到 5 个容器：
```
ppanel-mysql    (MySQL 8.0)
ppanel-redis    (Redis 7.0)
ppanel-server   (后端 API)
ppanel-admin    (管理后台)
ppanel-user     (用户前端)
```

### 查看日志

```bash
# 后端日志
docker compose -f /tmp/docker-compose-server.yml logs -f server

# 前端日志
docker compose -f /tmp/docker-compose-local.yml logs -f admin
docker compose -f /tmp/docker-compose-local.yml logs -f user

# 数据库日志
docker logs ppanel-mysql
docker logs ppanel-redis
```

### 重启服务

```bash
# 重启后端
docker compose -f /tmp/docker-compose-server.yml restart server

# 重启前端
docker compose -f /tmp/docker-compose-local.yml restart admin
docker compose -f /tmp/docker-compose-local.yml restart user
```

### 停止服务

```bash
# 停止后端
docker compose -f /tmp/docker-compose-server.yml down

# 停止前端
docker compose -f /tmp/docker-compose-local.yml down
```

---

## 🔄 更新应用

### 更新后端

```bash
cd ~/blitz-arrow-server
git pull
./scripts/deploy-from-source.sh
```

### 更新前端

```bash
cd ~/blitz-arrow
git pull
./scripts/deploy-from-source.sh
```

---

## 🔌 网络连接说明

### Docker 网络架构

```
ppanel-network (Docker 网络)
    ├── ppanel-mysql (mysql:3306)
    ├── ppanel-redis (redis:6379)
    ├── ppanel-server (server:8080)
    ├── ppanel-admin (admin:3000)
    └── ppanel-user (user:3000)
```

### 连接关系

```
1. 用户浏览器 → 前端容器
   http://136.110.11.215:3000 → ppanel-admin
   http://136.110.11.215:3001 → ppanel-user

2. 前端 JS 代码 → 后端 API
   http://136.110.11.215:8080 → ppanel-server
   （在用户浏览器中执行）

3. 后端容器 → 数据库容器
   mysql:3306 → ppanel-mysql
   redis:6379 → ppanel-redis
   （在 Docker 网络内部）
```

### 为什么后端用服务名？

```bash
# ✅ 正确 - 容器内使用服务名
PPANEL_DB=ppanel:ppanel_password@tcp(mysql:3306)/ppanel

# ❌ 错误 - 不能用 localhost
PPANEL_DB=ppanel:ppanel_password@tcp(localhost:3306)/ppanel
```

因为在 Docker 网络中，容器之间通过服务名通信。

### 为什么前端用公网 IP？

```bash
# ✅ 正确 - 用户浏览器访问
NEXT_PUBLIC_API_URL=http://136.110.11.215:8080

# ❌ 错误 - 浏览器无法访问
NEXT_PUBLIC_API_URL=http://server:8080
```

因为前端 API 调用在用户浏览器中执行，需要公网可访问的地址。

---

## 🔐 安全配置

### 1. 修改默认密码

**生产环境必须修改！**

```bash
# 修改 MySQL 密码
docker exec -it ppanel-mysql mysql -uroot -pppanel_root_password
ALTER USER 'ppanel'@'%' IDENTIFIED BY 'NEW_STRONG_PASSWORD';
FLUSH PRIVILEGES;
```

### 2. 限制防火墙访问

```bash
# 只允许特定 IP 访问
gcloud compute firewall-rules update allow-ppanel-all \
  --source-ranges YOUR_IP/32
```

### 3. 启用 HTTPS

使用 Nginx 反向代理 + Let's Encrypt：

```bash
sudo apt install nginx certbot python3-certbot-nginx
```

---

## 📂 目录结构

```
~/ (虚拟机家目录)
├── blitz-arrow-server/          # 后端项目
│   ├── bin/
│   ├── cmd/
│   ├── internal/
│   └── scripts/
│       └── deploy-from-source.sh
└── blitz-arrow/                 # 前端项目
    ├── apps/
    │   ├── admin/
    │   └── user/
    └── scripts/
        ├── deploy-from-source.sh
        └── configure-for-vm.sh
```

---

## 🆘 常见问题

### Q1: 前端无法连接后端

**检查清单**：
1. 后端是否正常运行：`docker ps | grep ppanel-server`
2. 防火墙是否开放 8080：访问 http://136.110.11.215:8080
3. 前端配置是否正确：`cat apps/admin/.env.local`
4. 浏览器开发者工具查看网络请求

### Q2: 后端无法连接数据库

**检查清单**：
1. 容器是否在同一网络：`docker network inspect ppanel-network`
2. 数据库是否健康：`docker logs ppanel-mysql`
3. 配置是否使用服务名：应该是 `mysql` 不是 `localhost`

### Q3: 容器启动失败

```bash
# 查看详细错误
docker logs ppanel-server
docker logs ppanel-admin

# 检查端口占用
sudo netstat -tlnp | grep -E "3000|3001|8080"

# 重新部署
docker compose down
./scripts/deploy-from-source.sh
```

### Q4: 修改 IP 地址后如何更新？

```bash
# 1. 更新脚本中的 IP
vim scripts/configure-for-vm.sh  # 修改 VM_IP
vim scripts/deploy-from-source.sh  # 修改 VM_IP

# 2. 重新配置前端
cd ~/blitz-arrow
./scripts/configure-for-vm.sh

# 3. 重新部署前端
./scripts/deploy-from-source.sh
```

---

## 📊 性能优化

### 虚拟机配置建议

| 配置 | 最低 | 推荐 |
|------|------|------|
| CPU | 2 vCPU | 4 vCPU |
| 内存 | 4GB | 8GB |
| 磁盘 | 20GB | 50GB |

### 资源占用参考

```
ppanel-mysql    ~500MB
ppanel-redis    ~50MB
ppanel-server   ~100MB
ppanel-admin    ~100MB
ppanel-user     ~100MB
-------------------
总计            ~850MB
```

---

## 🎉 部署完成检查清单

- [ ] 后端容器运行正常
- [ ] 数据库初始化完成
- [ ] 前端容器运行正常
- [ ] 防火墙规则已配置
- [ ] 可以访问 Admin: http://136.110.11.215:3000
- [ ] 可以访问 User: http://136.110.11.215:3001
- [ ] 可以登录管理后台
- [ ] API 调用正常

---

祝部署顺利！🚀

如有问题，查看日志：
```bash
docker logs ppanel-server
docker logs ppanel-admin
docker logs ppanel-user
```

